...
from sklearn.metrics import (
    accuracy_score, auc, precision_recall_curve, precision_score, recall_score, f1_score, roc_auc_score
)
...
def train_single_config(config, train_loader, val_loader, device, epochs=20):
    ...
    #pos_weight = torch.tensor([(77.62/22.38) ** 0.5], device=device)

    criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    optimizer = optim.Adam(model_lstm.parameters(), lr=config['lr'])
    ...
    for epoch in range(epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            logits = model_lstm(batch_X) # Usa internamente el squeeze(-1)
    ...
    return avg_val_loss, model_lstm, logs, y_true_epoch, y_pred_epoch, y_prob_epoch

def find_best_threshold(y_true_val, y_prob_val):
    """
    Encuentra el umbral óptimo en el set de validación que maximiza el F1-Score 
    para la Clase 1 (paquetes recibidos correctamente / colisiones).
    """
    precisions, recalls, thresholds = precision_recall_curve(y_true_val, y_prob_val)
    
    # Calcular F1-Score para cada umbral posible
    # Añadimos un pequeño epsilon (1e-10) para evitar divisiones por cero
    f1_scores = (2 * precisions * recalls) / (precisions + recalls + 1e-10)
    
    # Encontrar el índice del mejor F1-Score
    best_idx = np.argmax(f1_scores)
    
    # Control de seguridad por si el índice coincide con el final del array
...
param_grid = {
    'seq_length': [8],
    'hidden_size' : [32], #128                # Dimension of the mode
    'num_layers'  : [1, 2, 3, 4, 5],                    # Number of encoder layers
...
param_grid = {
    ...
    'dropout'     : [0.3, 0.4, 0.5], # , 0.2                # doprout
    'lr'          : [0.001, 0.0005], #, 0.0005            # Learning rate
    'batch_size'  : [32]                          # Batch Size
}
...
for i, config in enumerate(combinations):
    ...
    for seed in range(num_seeds):
        ...
        X_train, y_train, X_test, y_test, X_val  , y_val, scaler = load_and_prepare_data(seq_length = config['seq_length'], global_path_train=path_train, global_path_test=path_test, global_path_val=path_val)
        ...
        final_loss, model_lstm, logs, y_true, y_pred, _ = train_single_config(config, train_loader, val_loader, device, epochs=20)
...
pd.DataFrame(gs_results).to_csv('bilstm_gridsearch_results.csv', index=False)
print(f"Best config: {best_config}  (mean val_loss={best_gs_loss:.4f})")
print("Grid search results saved to 'bilstm_gridsearch_results.csv'.")
...
for seed in range(num_seeds):
    ...
    X_train, y_train, X_test, y_test, X_val  , y_val, scaler = load_and_prepare_data(seq_length = best_config['seq_length'], global_path_train=path_train, global_path_test=path_test, global_path_val=path_val)
    ...
    # Training
    val_loss, model_lstm, logs, y_true_val, y_pred_val, y_prob_val = train_single_config(best_config, train_loader, val_loader, device, epochs=20)
    ...
    model_path  = os.path.join('models/', f'bilstm_best_seed{seed}.pth')
    scaler_path = os.path.join('models/', f'bilstm_scaler_seed{seed}.pkl')
...
df_results.to_csv('baselines_bilstm_results.csv', index=False)

df_results = pd.DataFrame(results_all_seeds_best_threshold)
df_results.to_csv('baselines_bilstm_results_best_threshold.csv', index=False)

print("Results saved to 'baselines_bilstm_results.csv'.")    