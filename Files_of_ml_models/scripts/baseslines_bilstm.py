...
def train_single_config(config, train_loader, val_loader, device, epochs=20):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for batch_X, batch_y in val_loader:
                ...
                # Save probabilites and predictions for metrics calculation
    ...
    return avg_val_loss, model_lstm, logs, y_true_epoch, y_pred_epoch
...
param_grid = {
    ...
    'num_layers'  : [2, 3, 4, 5],                    # Number of encoder layers
    'dropout'     : [0.3, 0.4, 0.5, 0.6, 0.7], # , 0.2                # doprout
    'batch_size'  : [32]                          # Batch Size
}
'''
param_grid = {
    'hidden_size' : [32], #128                # Dimension of the mode
    'num_layers'  : [1, 2],                    # Number of encoder layers
    'dropout'     : [0.1], # , 0.2                # doprout
    'batch_size'  : [32]                          # Batch Size
}
'''
...
for i, config in enumerate(combinations):
    ...
    for seed in range(num_seeds):
        ...
        # Training
        final_loss, model_lstm, logs, y_true, y_pred = train_single_config(config, train_loader, val_loader, device, epochs=20)
...
for seed in range(num_seeds):
    ...
    # Training
    val_loss, model_lstm, logs, y_true_val, y_prob_val = train_single_config(best_config, train_loader, val_loader, device, epochs=20)
